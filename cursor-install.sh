#!/usr/bin/env bash
#
# Install the Memco Shared Memory plugin into a local Cursor setup, straight from
# this repo.
#
...
echo
echo "✓ Installed Memco Shared Memory${VERSION:+ (v${VERSION})} to:"
echo "    ${DEST}"
echo
echo "Next steps in Cursor:"
echo "  1. Reload: Cmd/Ctrl+Shift+P → 'Developer: Reload Window'  (or restart Cursor)"
echo "  2. Open Cursor Settings → Plugins to confirm 'Memco Shared Memory' is listed"
echo "  3. Authenticate Memco Shared Memory in the browser when prompted (OAuth)"
